...
namespace Worlds
{
    ...
    /// </summary>
    [SkipLocalsInit]
    public unsafe ref struct ComponentQuery<C1, C2, C3, C4, C5, C6> where C1 : unmanaged where C2 : unmanaged where C3 : unmanaged where C4 : unmanaged where C5 : unmanaged where C6 : unmanaged
    {
        ...
        public ref struct Enumerator
        {
            ...
            internal Enumerator(ComponentQuery<C1, C2, C3, C4, C5, C6> query)
            {
                ...
                if (chunkCount > 0)
                {
                    chunks = new(MemoryAddress.Allocate(chunksBuffer.Slice(0, chunkCount)), chunkCount);
                    ChunkPointer* chunk = chunksBuffer[0].chunk;
